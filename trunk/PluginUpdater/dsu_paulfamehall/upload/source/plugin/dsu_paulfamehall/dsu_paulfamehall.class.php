<?php
/*
	dsu_paulfamehall_ECHO By shy9000 2010-08-10
*/
		...
		if(is_array($postlist)) {
			foreach($postlist as $key => $val) {
				if($val['author'] && isset($this->fhc[$val['authorid']])) {
					...
					$return[] = "<p id=\"famehall_".$val['pid']."\" onmouseover=\"showMenu({'ctrlid':this.id, 'pos':'12'});\"><a href=\"plugin.php?id=dsu_paulfamehall:star&gid={$pgid}\"><img src=\"source/plugin/dsu_paulfamehall/gpic/".$pic."\" /></a></p><div id=\"famehall_".$val['pid']."_menu\" class=\"g_up\" style=\"display:none\"><div class=\"crly\"><center>".lang('plugin/dsu_paulfamehall','gxs_01')." <font color=\"red\">{$pfc}</font>.<br>".lang('plugin/dsu_paulfamehall','admin_54').": <font color=\"red\">{$pjs}</font></center></div><div class=\"mncr\"></div></div>";
				} else {
					$return[] = '';
				}
			}
