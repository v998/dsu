...
if(!class_exists('hott_script_hot_post_1')){
	class hott_script_hot_post_1{
		var $name;
		function _fetch_data($block_id) {
			...
			$query=DB::query("SELECT tid,highlight FROM {$tablepre}forum_thread WHERE tid>0 {$only_lz}{$limitforum}{$date_limit}{$show_group} ORDER BY views LIMIT 0,{$limit}");
			$tids='';
		...
		function hott_script_hot_post_1(){
			global $hott_lang;
			$hott_lang && !$this->name && $this->name=$hott_lang['block_hot_post_1'];
		}
		function output($block_id){
			global $_G,$hott,$threadid,$config,$authorid,$hott;
			loadcache('dsu_hott');
			$cache=$_G['cache']['dsu_hott']['hott_script_hot_post_1'];
			if(TIMESTAMP-$cache['updatetime']>$hott[$block_id]['cache_time'] || $hott[$block_id]['only_lz']){
				$data=$this->_fetch_data($block_id);
				$data['updatetime']=TIMESTAMP;
				$_G['cache']['dsu_hott']['hott_script_hot_post_1']=$cache=$data;
...
$hott_script=new hott_script_hot_post_1;
