...
if(!class_exists('hott_script_hot_post_2')){
	class hott_script_hot_post_2{
		...
		function _fetch_data($block_id) {
			...
			$query=DB::query("SELECT tid,highlight,subject FROM {$tablepre}forum_thread WHERE displayorder>-1 {$only_lz}{$limitforum}{$date_limit}{$show_group} ORDER BY replies DESC LIMIT 0,{$limit}");
