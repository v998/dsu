...
if(!class_exists('hott_script_digest')){
	class hott_script_digest{
		...
		function _fetch_data($block_id) {
			...
			$query=DB::query("SELECT tid,highlight FROM {$tablepre}forum_thread WHERE tid>0 {$only_lz}{$limitforum}{$date_limit}{$show_group} AND digest>0 ORDER BY digest LIMIT 0,{$limit}");
			$tids='';
		...
		function hott_script_digest(){
			global $hott_lang;
			$hott_lang && !$this->name && $this->name=$hott_lang['block_digest'];
		}
