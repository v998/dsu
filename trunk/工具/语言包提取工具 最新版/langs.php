...
while(false !== ($entry = $d->read())) {
	...
	if($extend[$va] == 'php') {
		...
		$c = preg_replace('/(showmessage|cpmsg)\("([\xA0-\xFF]+[^"]+[\xA0-\xFF<>\.]+)"/e', 'returnstr_php(\'\2\', "$file[basename]","\1")', $c);
		$c = preg_replace("/(showmessage|cpmsg)\('([\xA0-\xFF]+[^']*[\xA0-\xFF<>\.]+)'/e", 'returnstr_php(\'\2\', "$file[basename]","\1")', $c);
		$c = preg_replace('/"([\xA0-\xFF]+[^"]+[\xA0-\xFF<>\.]+)"/e', 'returnstr_php(\'\1\', "$file[basename]")', $c);
		$c = preg_replace("/'([\xA0-\xFF]+[^']+[\xA0-\xFF<>\.>]+)'/e", 'returnstr_php(\'\1\', "$file[basename]")', $c);
