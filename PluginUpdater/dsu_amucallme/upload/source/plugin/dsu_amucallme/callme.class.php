...
class plugin_dsu_amucallme{

	function plugin_dsu_amucallme(){
		global $_G;
		$this->mvars = $_G['cache']['plugin']['dsu_amucallme'];
...
class plugin_dsu_amucallme_forum extends plugin_dsu_amucallme {
	...
	function post_middle_output($a) {
		...
		if($_G['uid']){
			...
			$str .= '$("e_adv_s1").innerHTML += \'<div class="b1r" id="dsu_amucallme_tag"><a href="plugin.php?id=dsu_amucallme:callme&adds=e_iframe" onclick="showWindow(\\\'dsu_amucallme_add\\\', this.href);" title="'.lang('plugin/dsu_amucallme','callme').'">'.lang('plugin/dsu_amucallme','callme').'</a></div>\';';
			$str .= '</script>';
			$css = '<style>#dsu_amucallme_tag {border:1px solid #F5F5F5;float:left;overflow:hidden;}';
			$css .= '#dsu_amucallme_tag a {background:url("source/plugin/dsu_amucallme/images/a1.png") no-repeat scroll 0 0 transparent;background-position:center  1px;border:1px solid #F5F5F5;float:left;overflow:hidden;}';
			$css .= '#dsu_amucallme_tag a:hover {background-color:#FFFFFF;border-color:#0099CC;text-decoration:none;}</style>';
		}
		return $str.$css;
	}
	...
	function viewthread_postfooter_output(){		
		...
		if($_G['uid']){
			foreach ($postlist as $value){
				...
				if($author <> $username){
					$authors[] = $author;
					$shows[]= "<a class='add_callme' href='javascript:;' onclick='seditor_insertunit(\"fastpost\", \"[@]".$author."[/@]\");window.scrollTo(0,99999);'>".lang('plugin/dsu_amucallme','callme')."</a>";
				}else{$shows[]= '';}
			}
			if(array_unique($authors)){
				$authors = serialize(array_unique($authors));
				dsetcookie('dsu_amucallme_'.$this->tp, base64_encode($authors), 600);
				dsetcookie('dsu_amucallme_lz'.$this->tp, base64_encode($_G['forum_thread']['author']), 600);
			}
			//var_dump($shows);
		}
		return (array)$shows;
	...
	function viewthread_fastpost_ctrl_extra() {
		...
		if($_G['uid']){
			...
			$str = '<div class="dsu_amucallme_tag"><a href="plugin.php?id=dsu_amucallme:callme&adds=fastpostmessage&tp='.$this->tp.'" title="'.lang("plugin/dsu_amucallme","callme").'" onclick="showWindow(\'dsu_amucallme_add\', this.href);">'.lang("plugin/dsu_amucallme","callme").'</a></div>';
			$css = '<style>';
			$css .= '.dsu_amucallme_tag a {background:url("source/plugin/dsu_amucallme/images/a1.png") no-repeat scroll 0 0 transparent;float:left;height:20px;line-height:20px;margin:4px 4px 0px 4px;overflow:hidden;text-indent:-9999px;width:20px;}';
			$css .= '.add_callme{background:url(source/plugin/dsu_amucallme/images/a1.png) no-repeat 0 50%;}</style>';
		}
		return $str.$css;
	}
	...
	function post_amucallme(){
		...
		if($_G['uid'] && in_array($_G['fid'],$this->fids)){
			...
			dsetcookie('dsu_amucallme_ed', base64_encode($authorsd), 600);
			if($pp["username"]){
