...
/*
	...
	$Id: admin_manage.inc.php 22 2011-01-04 16:41:08Z chuzhaowei@gmail.com $
*/
...
if(empty($_G['gp_pdo']) || $_G['gp_pdo'] == 'list'){ //列表页面
	if(!submitcheck('medalsubmit')) {
		showtips(lang('plugin/dsu_medalCenter','9'));
...
<?
		...
		while($medal = DB::fetch($query)) {
			...
			switch($medal['type']) {
				...
				case 2:
					...
					break;
				case 5:
					$medal['type'] = lang('plugin/dsu_medalCenter','13');
					break;
			}
			showtablerow('', array('class="td25"', 'class="td25"', 'class="td25"', '', '', '', 'class="td23"', ''), array(
				...
				$medal[type],
				"<a href=\"".ADMINSCRIPT."?action=plugins&operation=config&identifier=dsu_medalCenter&pmod=admin_manage&pdo=edit&medalid=$medal[medalid]\" class=\"act\">".lang('plugin/dsu_medalCenter','10')."</a> | ".
				"<a href=\"".ADMINSCRIPT."?action=plugins&operation=config&identifier=dsu_medalCenter&pmod=admin_manage&pdo=copy&medalid=$medal[medalid]\" class=\"act\">".lang('plugin/dsu_medalCenter','11')."</a>"
			));
		}
	...
	if(!submitcheck('medaleditsubmit')) {
		...
		$typevar[1][] = array(0, lang('plugin/dsu_medalCenter','12'));
		...
		showsetting('medals_type1', '', '', '<ul class="nofloat" onmouseover="altStyle(this);">
			...
			<li'.($checkmedaltype[5] ? ' class="checked"' : '').'><input name="typenew" onclick="$(\'creditbody\').style.display = this.checked ? \'\' : \'none\'" type="radio" class="radio" value="5" '.$checkmedaltype[5].'>&nbsp;'.lang('plugin/dsu_medalCenter','13').'</li></ul>'
		);
		showsetting('medals_expr1', 'expirationnew', $medal['expiration'], 'number');
		showsetting(lang('plugin/dsu_medalCenter','5'), $typevar, $medal['typeid'], 'select');
		...
		updatecache('medals');
		$msg = $_G['gp_pdo'] == 'edit' ? 'medals_succeed' : lang('plugin/dsu_medalCenter','14');
