...
if(submitcheck('typesubmit')){
	...
	cpmsg(lang('plugin/dsu_medalCenter','4'), 'action=plugins&operation=config&identifier=dsu_medalCenter&pmod=admin_type', 'succeed');
}else{
	showformheader("plugins&operation=config&identifier=dsu_medalCenter&pmod=admin_type", '', 'configform');
	showtableheader(lang('plugin/dsu_medalCenter','5'));
	showtablerow('class="header"', array('class="td25"','class="td28"',''), array(
		cplang(''),
		cplang(lang('plugin/dsu_medalCenter','6')),
		cplang(lang('plugin/dsu_medalCenter','7')),
	));
...
EOT;
	echo '<tr><td></td><td colspan="8"><div><a href="###" onclick="addrow(this, 0)" class="addtr">'.lang('plugin/dsu_medalCenter','8').'</a></div></td></tr>';
